...
@interface StockDetailViewController () <UITextFieldDelegate>
...
- (id)initWithNASDAQSymbol:(NSString *)string {
    _symbol = string;
    return [self initWithNibName:nil bundle:nil NASDAQSymbol:string];
}

- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil NASDAQSymbol:(NSString *)symbol {
...
- (void)viewDidLoad {
    ...
    // since in init we do setup already
    [self setupUI];
}
...
//    @"http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.quotes%20where%20symbol%20in%20%20%28%22AAPL%22%2C%22GOOG%22%2C%22MSFT%22%29&diagnostics=true&env=http%3A%2F%2Fdatatables.org%2Falltables.env&format=json";//&format=json
    ...
    // iphone finance-yql.media.yahoo.com/72.30.202.51
        ...
        if ([data length] > 0 && error == nil) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^(void){
                if ([[[[[dict objectForKey:@"query"] objectForKey:@"results"] objectForKey:@"quote"] objectAtIndex:0] objectForKey:@"LastTradePriceOnly"] == [NSNull null]) {
                    ...
                    NSString *combinedStringWithChangeAndPercentage = [[[[[dict objectForKey:@"query"] objectForKey:@"results"] objectForKey:@"quote"] objectAtIndex:0] objectForKey:@"Change_PercentChange"];
                    //TODO: split change and percentage.
                    _changeField.text = [StockAPIUtil getValidString:[[[[[dict objectForKey:@"query"] objectForKey:@"results"] objectForKey:@"quote"] objectAtIndex:0] objectForKey:@"Change"]];  //  Change_PercentChange
...
//                    _targetPriceField.text = [StockAPIUtil getValidRatio:[[[[[dict objectForKey:@"query"] objectForKey:@"results"] objectForKey:@"quote"] objectAtIndex:0] objectForKey:@""]];// OneyrTargetPrice
                    ...
                    _mktCapField.text = [StockAPIUtil getValidString:[[[[[dict objectForKey:@"query"] objectForKey:@"results"] objectForKey:@"quote"] objectAtIndex:0] objectForKey:@"MarketCapitalization"]];
                }
            });
...
- (void)addJVFloatLabel {
    ...
    _titleField.text = _symbol != nil?_symbol:@"";
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    _mute = [NSTimer scheduledTimerWithTimeInterval:5.0f target:self selector:@selector(checkIfMuteIsOn) userInfo:nil repeats:YES];
    [_mute invalidate];
//    [_mute fire];
